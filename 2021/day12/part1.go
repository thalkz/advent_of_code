...
func walk(current *Cave, visited []string) int {
	fmt.Printf("Walked in %v. Has visited %v. Can visit: ", current, visited)

	for i := range current.links {
		if !isVisited(current.links[i].name, visited) {
			fmt.Printf("%v ", current.links[i].name)
		}
	}
	fmt.Println()
	...
	for i := range current.links {
		if !isVisited(current.links[i].name, visited) {
			total += walk(current.links[i], visited)
		}
	}
	return total
}
