...
func propagate(_ light: Point, _ visited: inout Set<Point>) {
    ...
    for dir in outputDirs {
        propagate(move(light.x, light.y, dir), &visited)
    }
}

func count(_ visited: Set<Point>) -> Int {
...
for j in 0...(grid.count-1) {
    ...
    propagate(Point(x:0, y:j, dir:.right), &visited)
    maxEnergy = max(count(visited), maxEnergy)
}

for j in 0...(grid.count-1) {
    var visited: Set<Point> = []
    propagate(Point(x:grid[0].count-1, y:j, dir:.left), &visited)
    maxEnergy = max(count(visited), maxEnergy)
}

for i in 0...(grid[0].count-1) {
    var visited: Set<Point> = []
    propagate(Point(x:i, y:0, dir:.down), &visited)
    maxEnergy = max(count(visited), maxEnergy)
}

for i in 0...(grid[0].count-1) {
    var visited: Set<Point> = []
    propagate(Point(x:i, y:grid.count-1, dir:.up), &visited)
    maxEnergy = max(count(visited), maxEnergy)
}
